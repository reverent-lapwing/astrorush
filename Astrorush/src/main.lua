function love.load()
	...
	mfont = love.graphics.setNewFont("pcsenior.ttf",16)
	mbfont = love.graphics.setNewFont("pcsenior.ttf",36)
	...
	fulscreen = love.graphics.newImage("fullscreen.png")
...
function love.update(e)
	...
	if bob then
		if mus1:tell()>200 then
			love.audio.play(mus2)
			love.audio.stop(mus1)
			
			bob = nil
		end
	else
		if mus2:tell() > 173 then
			mus2:seek(0)
		end
	end
...
function love.draw()
	...
	if mute then
		...
		love.graphics.draw(speakerOn,screenWidth-92,0)
	end
	
	love.graphics.draw(boom,screenWidth-41,0)
--	love.graphics.draw(fulscreen,screenWidth-41,0)
