...
function enable_handlers(obj)
  if obj.update then
  handlers.update[obj.id] = obj
  end
  if obj.draw then
  handlers.draw[obj.id] = obj
  end
  if obj.mousepressed then
  handlers.mousepressed[obj.id] = obj
  end
  if obj.mousereleased then
  handlers.mousereleased[obj.id] = obj
  end
  if obj.keypressed then
  handlers.keypressed[obj.id] = obj
  end
  if obj.keyreleased then
  handlers.keyreleased[obj.id] = obj
  end
  if obj.textinput then
  handlers.textinput[obj.id] = obj
  end
  if obj.focus then
  handlers.focus[obj.id] = obj
  end
...
function disable_handlers(obj)
   if handlers.update[obj.id] then 
   table.remove(handlers.update, obj.id)
   end
   if handlers.draw[obj.id] then 
   table.remove(handlers.draw, obj.id)
   end
   if handlers.mousepressed[obj.id] then 
   table.remove(handlers.mousepressed, obj.id)
   end
   if handlers.mousereleased[obj.id] then 
   table.remove(handlers.mousereleased, obj.id)
   end
   if handlers.keypressed[obj.id] then 
   table.remove(handlers.keypressed, obj.id)
   end
   if handlers.keyreleased[obj.id] then 
   table.remove(handlers.keyreleased, obj.id)
   end
   if handlers.textinput[obj.id] then 
   table.remove(handlers.textinput, obj.id)
   end
   if handlers.focus[obj.id] then 
   table.remove(handlers.focus, obj.id)
   end
